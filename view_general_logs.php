...
try {
    $sql = "SELECT * FROM nw_log_login_gl ORDER BY time DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $logs = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    echo "Error fetching logs: " . $e->getMessage();
    $logs = []; // Ensure $logs is an array even if the fetch fails
}
/*
                                ...
                                <table class="table align-items-center mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            switch ($log['action']) {
                                            case 1:
                                                echo '<i class="fas fa-check-circle text-success"></i> Login Success';
                                            break;
                                            case 2:
                                                echo '<i class="fas fa-times-circle text-danger"></i> Wrong Password';
                                            break;
                                            case 3:
                                                echo '<i class="fas fa-exclamation-circle text-warning"></i> Wrong Username or Password';
                                            break;
                                            default:
                                            echo '<i class="fas fa-question-circle text-secondary"></i> Unknown Action';
                                                                   }
                                        ?></td>
                                        </tr>
                                        <?php endforeach; ?>
